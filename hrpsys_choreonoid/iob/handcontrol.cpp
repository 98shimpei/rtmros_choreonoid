...
bool handcontrolPlugin_impl::setup(robot_state *rs,motor_command *mc) {
  ...
  set_number_of_joints(DOF);//iob
  set_number_of_force_sensors(0);//iob
  set_number_of_gyro_sensors(0);//iob
  set_number_of_accelerometers(0);//iob

  set_signal_period(long(1e9 * m_dt));//iob
  open_iob();//iob
  ...
  std::vector<double> vec;
  {
    ...
    for(int i = 0; i < vec.size(); i++) {
      vec[i] = Servo[i].f_limit;
    }
  }
  iob_set_torque_limit(vec);//iob
...
void setServoInfo(void)
{
  ...
  for (i = 0 ; i < MOTOR_NO; i++ ) {
    ...
    write_pgain(i,50.0);//iob
    write_dgain(i,10.0);//iob
    write_servo(i,0);//iob
    write_power_command(i,0);//iob
  }
